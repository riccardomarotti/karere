...
- **Stale Chromium references in `.github/workflows/build-cef-codecs.yml` synced to 150 (KARE-013)**: the `cef_branch` workflow_dispatch input description and the upload-artifact name now say Chromium 150 / `cef-150-proprietary-codecs-linux64` instead of the retired 148 line; `tests/cef_version_refs.sh` pins the workflow's version references to the shipping Chromium major.
...
### Fixed
- **GNOME OS / Wayland black window (#164), properly this time**: even with the 4.1.1 GL-renderer change the window was still intermittently black (a startup race). CEF was auto-selecting the X11/Xwayland Ozone backend, which can't present onto the Wayland view. Karere now forces CEF's Ozone backend to match the display server GTK uses, removing the race.
...
### Changed
- **Updated the bundled browser engine to CEF 149 (Chromium 149.0.7827)**, up from CEF 148. This picks up the latest Chromium security and rendering fixes. Proprietary codecs (H.264/AAC) remain enabled so WhatsApp audio/video calls and MP4 playback keep working.
